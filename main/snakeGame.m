...
% This program create a function snakeGame to run the snake game
...
% Update 5: Prompt the user for which level they want to choose and set the
% speed of snake based on their choice by using level() function

% Update 6: Add messagebox for end game callback function

% Update 7: Update figure with more playful colors.

%-------------------------------------------------------------------------%

function snakeGame()
    global fig axes1 score_num;
    ...
    function start_game_Callback(src, event)
        ...
        % Prompt the user for level and set timer for each level
        level();
    ...
    function end_game_Callback(src, event)
        closeChoice = uiconfirm(fig, 'Are you sure you want to end the game?', ...
            'End Game', 'Options', {'Yes', 'No'}, 'DefaultOption', 2);
        % If user confirms, close the figure
        if strcmp(closeChoice, 'Yes')
